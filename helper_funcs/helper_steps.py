...
def parse_to_meaning_ful_text(input_phone_number, in_dict):
    ...
    me_t += "\n"
    me_t += "<b>APP ID</b>: "
    me_t += f'<code>{in_dict["App Configuration"]["app_id"]}</code>'
    me_t += "\n"
    me_t += "<b>API HASH</b>: "
    me_t += f'<code>{in_dict["App Configuration"]["api_hash"]}</code>'
    me_t += "\n"
    me_t += "\n"
    me_t += "<i>Available MTProto Servers</i>"
    me_t += "\n"
    me_t += "<b>Production Configuration</b>: "
    me_t += f'<code>{in_dict["Available MTProto Servers"]["production_configuration"]["IP"]}</code> <u>{in_dict["Available MTProto Servers"]["production_configuration"]["DC"]}</u>'

    me_t += "\n"
    me_t += "<b>Test Configuration</b>: "
    me_t += f'<code>{in_dict["Available MTProto Servers"]["test_configuration"]["IP"]}</code> <u>{in_dict["Available MTProto Servers"]["test_configuration"]["DC"]}</u>'

    me_t += "\n"
    me_t += "\n"
    me_t += "<i>Disclaimer</i>: "
    me_t += f'<u>{in_dict["Disclaimer"]}</u>'
    return me_t
